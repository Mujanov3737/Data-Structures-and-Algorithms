...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done)
    {
        ...
        {
            bids.at(low).title.swap(bids.at(high).title);
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    quickSort(bids, begin, midpoint - 1);
...
void selectionSort(vector<Bid>& bids) {
    ...
    for (size_t pos = 0; pos < vsize - 1; ++pos)
    {
        min = pos;
        // loop over remaining elements to the right of position
        for (size_t i = pos + 1; i < vsize; ++i) {
       ...
       // swap is a built in vector method
        bids.at(min).title.swap(bids.at(pos).title);
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 1:
            ...
            cout << "time: " << ticks * 1.0000 / CLOCKS_PER_SEC << " seconds" << endl;
        ...
        case 3:
            ...
            // Invoke the selection sort
            selectionSort(bids);

            // Displaying number of bids sorted
